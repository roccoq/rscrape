...
class TestWebscrape(unittest.TestCase):
    ...
    def test_updatewebformdata(self):
    ...
    def test_updatewebformdata_empty_inputs(self):
    ...
    def test_processrepeaterdata_basic_fm(self):
        ...
        processrepeaterdata(
            rpters, repeater_list, rfilter, chirp, chirpcount, chirprepeater,
            chirprepeaterlist, searchfilter, exnotes, DEBUG, tx_power, ams_mode
        )
    ...
    def test_processrepeaterdata_ysf_mode(self):
    ...
    def test_processrepeaterdata_chirp_output(self):
    ...
    def test_processrepeaterdata_search_filter(self):
        """Test search filter to include only matching entries."""
        rpters = [["City, ST", "145.0", "100.0", "CALL", "10.0N", "Sponsor", "NB1RI Notes"]]  # freq as str to avoid type error
        repeater_list = []
        rfilter = ["all"]
        chirp = False
        chirpcount = 0
    ...
    def test_processrepeaterdata_nan_handling(self):
        """Test handling of 'nan' in raw data."""
        rpters = [[float('nan'), float('nan'), float('nan'), float('nan'), "10.0N", "Sponsor", float('nan')]]
        repeater_list = []
        rfilter = ["all"]
        chirp = False
        chirpcount = 0
    ...
    def test_processrepeaterdata_dcs_code(self):
    ...
    def test_processrepeaterdata_extended_notes(self):
        """Test inclusion of extended notes."""
        rpters = [["City, ST", 145.0, "100.0", "CALL", "10.0N", "Sponsor", "Extra Notes"]]
        repeater_list = []
        rfilter = ["all"]
        chirp = False
        chirpcount = 0
    ...
    def test_determineoffset_various_bands(self):
    ...
    def test_determineoffset_invalid(self):
    ...
    def test_filteroutput_multiple_modes(self):
    ...
    def test_filteroutput_single_mode(self):
    ...
    def test_chirpbuild(self):
        """Test appending CHIRP entry."""
        chirprepeater = ["0", "CALL", 145.0, "-", "0.600000", "Tone", "100.0", "100.0", "023", "NN", "FM", "5.00", "", "Comment", "", "", "", ""]
        chirprepeaterlist = []
    ...
    @patch('sys.argv', ['webscrape.py', '-c', 'TestCity', '-s', 'TS', '-r', '10', '-b', '144'])
    @patch('logging.basicConfig')
    def test_main_basic(self, mock_logging, mock_open, mock_read_html, mock_post):
        ...
        mock_read_html.return_value = [pd.DataFrame(), mock_df]  # Simulate tables[0], tables[1]
    ...
    def test_main_debug_logging(self, mock_read_html, mock_post):
        ...
        mock_read_html.return_value = [pd.DataFrame(), mock_df]  # For both nerep and nyrep
